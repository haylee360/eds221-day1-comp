...
# takes everything from the first call and passes it directly into the next function
...
```{r}
# read in file ----
toolik_chlorophyll <- read.csv(here::here("data", "data-raw", "toolik_chlorophyll.csv"))
```
...
```{r}
toolik_chlorophyll$active_chl_a_ug_l <- na_if(toolik_chlorophyll$active_chl_a_ug_l, ".")
# right now active chloro conc is bein stored as character
...
```{r}
ggplot(toolik_chlorophyll, aes(x = depth, y = active_chl_a_ug_l)) +
  geom_point(aes(colour = active_chl_a_ug_l), show.legend = FALSE) +
  scale_color_gradient(low = "blue", high = "pink") +
  theme_minimal() +
  labs( title = "Active chlorophyll concentration by depth", 
        x = "Depth (m)",
        y = "Active chlorophyll concentration (micrograms/liter)")
```
